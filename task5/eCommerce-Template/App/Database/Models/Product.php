...
class Product extends Model implements ConnectTo
{
    ...
    public function getProductBYSub($status = 1)
    {
    ...
    {
        $query = "SELECT  `products_specs`.*, `specs`.`name` FROM `products_specs`     JOIN `specs` ON `specs`.`id` = `products_specs`.`spec_id`
    ...
    {
        $query = "SELECT `review`.* CONCAT(`users`.`first_name` , ' ' , `users`.`last_name`) AS `full_name` FROM `reviews`
        JOIN `users`  ON `users`.`id` = `reviews`.`user_id` WHERE   `reviews`.`product_id` = ?";
        $stmt = $this->con->prepare($query);
        $stmt->bind_param('i', $this->id);
        $stmt->execute();
        return $stmt;
